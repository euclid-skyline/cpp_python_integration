...
int main()
{
    ...
    // ---------------------------------------------------------
    PyObject *moduleName = PyUnicode_FromString("random_value");
    ...
    // Get function
    PyObject *func = PyObject_GetAttrString(module, "get_value");
    if (!func || !PyCallable_Check(func))
    {
        std::cerr << "Function get_value() not found or not callable\n";
        Py_XDECREF(func);
        Py_DECREF(module);
        Py_Finalize();
        return 1;
    ...
    while (running)
    {
        ...
        if (result)
        {
            ...
            std::cout << "\033[8G" << value << "   " << std::flush;
        }
        else
        {
