...
int main()
{
    ...
    int row = 0;
    int col = 0;
    std::string message = "Message Holder - Update me from Python!";
    int max_rows, max_cols;
    getmaxyx(stdscr, max_rows, max_cols);
    int reveal = 0;

    PyInterface::bind("row", row);
    PyInterface::bind("col", col);
    PyInterface::bind("message", message);
    PyInterface::bind("max_rows", max_rows);
    PyInterface::bind("max_cols", max_cols);
    PyInterface::bind("reveal", reveal);
    ...
    while (running)
    {
        ...
        if (result)
        {
            ...
            resize_term(0, 0); // Let ncureses resize internal buffers
            getmaxyx(stdscr, max_rows, max_cols);
            // Clear screen
            erase();

            // Precompute once per frame
            std::size_t msg_len = message.size();
            std::size_t start = msg_len - reveal;
            if (start > msg_len)
                start = 0;

            // Draw in every column
            for (int x = 0; x < max_cols; ++x)
            {
                int color_index = (x % total_colors) + 1; // cycle through 6 colors
                attron(COLOR_PAIR(color_index));

                for (std::size_t i = start; i < msg_len; ++i)
                {
                    int y = row + static_cast<int>(i - start);
                    if (y >= 0 && y < max_rows)
